...
h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2) # 30x30x32 -> 30x30x32
h_pool2 = max_pool_2x2(h_conv2) # 30x30x32 -> 15x15x32

h_pool2_flat = tf.reshape(h_pool2, [-1, 15*15*32])

drop = tf.nn.dropout(h_pool2_flat, keep_prob)

W_id = weight_variable([15 * 15 * 32, 256])
b_id = bias_variable([256])
pre_net = tf.nn.relu(tf.matmul(drop, W_id) + b_id)
net = tf.reshape(pre_net, [-1, 256])
...
gc.collect()
print "Loading data..."
data = np.reshape(np.loadtxt("../c10k_data.gz"), (amount, 60*60))
print "Loaded"
